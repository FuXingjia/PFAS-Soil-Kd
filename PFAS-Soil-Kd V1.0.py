...
"}")
        ...
        self.Retrain_tB.setStyleSheet("/* 默认 */\n"
"QToolButton{   \n"
"    border-top: 3px outset transparent;           /* 上边框宽度3像素、突出显示、颜色为透明 */\n"
"    border-bottom: 7px outset transparent;\n"
"    border-right: 3px outset transparent;\n"
...
"}")
        ...
        self.Descrip_tB.setStyleSheet("/* 默认 */\n"
"QToolButton{   \n"
"    border-top: 3px outset transparent;           /* 上边框宽度3像素、突出显示、颜色为透明 */\n"
"    border-bottom: 7px outset transparent;\n"
"    border-right: 3px outset transparent;\n"
...
"font: 12pt \"Times New Roman\";")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.Optimization_range_TextEdit.setPlainText(_translate("MainWindow", "{\'learning_rate\': (0.1,0.6),\n"
...
class MyMainForm(QMainWindow, Ui_MainWindow):
    ...
    def prediction(self):
        ...
        if len(Filename) != 0 and exists(Filename):
            ...
            if Model_choose == 'LightGBM trained by RDKit Dataset':
                ...
                if Data.shape[0] == 0 or Data.shape[1] != 22:
                    ...
                    results = DataFrame(result,
                                            columns=['SOC', 'CEC', 'Sand','Silt', 'Clay',
                                                     'MaxEStateIndex', 'PEOE_VSA4',	'Chi3v', 'FpDensityMorgan3', 'NumValenceElectrons',	'MinAbsEStateIndex', 'PEOE_VSA10', 'MaxPartialCharge',
                                                     'VSA_EState3',	'MinAbsPartialCharge',	'VSA_EState7',	'HallKierAlpha', 'MinPartialCharge', 'VSA_EState4',
                                                     'pH', 'PCS', 'logCe', 'pred_logKd'])
            ...
            if Model_choose == 'LightGBM trained by PaDEL Dataset':
                ...
                if Data.shape[0] == 0 or Data.shape[1] != 14:
                    ...
                    results = DataFrame(result,
                                           columns=['SOC', 'CEC', 'Sand', 'Silt', 'Clay',
                                                    'ALogP', 'ATS3s', 'SpMin8_Bhi', 'GATS2c', 'GATS3c', 'nRotBt',
                                                    'pH', 'PCS', 'logCe', 'pred_logKd'])
                    self.predict_text.setText('Model predict successfully! The number of predictive data is ' + str(len(input_data[:,0])) + '.')
                    self.tableWidget.setRowCount(results.shape[0])
                    self.tableWidget.setColumnCount(results.shape[1])
    ...
    def Bayesian_optimization(self):
        ...
        if len(path) != 0 and exists(path):
            ...
            def LGBM_evaluate(learning_rate, n_estimators,  max_depth, num_leaves, reg_alpha, reg_lambda, drop_rate, max_drop):
                if self.stop:
                ...
                if int(num_leaves) < 2 ** int(max_depth):
                    ...
                    self.Bayesian_textBrowser.append('Model parameter for ' + str(int(self.num)) + ' Iterations: ' + str(param))
                    result_r2 = []
                    result_mae = []
                    result_rmse = []
            ...
            if __name__ == '__main__':
                ...
                self.num = 0
                BayesianSearch(LGBM_evaluate, adj_params)
        ...
        if Single_SMILES != '':
            if Choose_MD == 'RDKit Descriptors':
                try:
                        ...
                        mol = MolFromSmiles(str(Single_SMILES))
                        ...
                        if RDKit_result.iloc[0,0] == -666.0:
                            ...
                            if Select_quantity == 'Just for Model':
                                descriptors_model = ['MaxEStateIndex', 'PEOE_VSA4',	'Chi3v', 'FpDensityMorgan3', 'NumValenceElectrons',	'MinAbsEStateIndex', 'PEOE_VSA10', 'MaxPartialCharge',
                                                        'VSA_EState3', 'MinAbsPartialCharge', 'VSA_EState7', 'HallKierAlpha', 'MinPartialCharge', 'VSA_EState4',
                                                        ]
                        ...
                        message = f"An exception of type {type(e).__name__} occurred.\nPlease ensure the SMILES is correct!"
            ...
            if Choose_MD == 'PaDEL Descriptors':
                try:
                        ...
                        self.MD_result_progressBar.setValue(1)
                        if Select_quantity == 'Just for Model':
                            descriptors_model = ['ALogP', 'ATS3s', 'SpMin8_Bhi', 'GATS2c', 'GATS3c', 'nRotBt']
                        ...
                        message = f"An exception of type {type(e).__name__} occurred.\nPlease ensure Java Runtime Environment and the SMILES is correct!"
                        QMessageBox.critical(self, "Error", message)
                        return False
                ...
                if len(smiles) != 0:
                        ...
                        if Choose_MD == 'PaDEL Descriptors':
                            try:
                                ...
                                for i in range(len(smiles)):
                                        result = from_smiles(smiles.iloc[i])
                                ...
                                message = f"An exception of type {type(e).__name__} occurred.\nPlease ensure Java Runtime Environment and check if the SMILES in the file are correct!"
                                QMessageBox.critical(self, "Error", message)
                                return False
...
if __name__ == "__main__":
    ...
    myWin = MyMainForm()
    myWin.setWindowIcon(QIcon('/icon/PFAS_Soil.ico'))
    myWin.show()
    myWin.click_button()
    sys.exit(app.exec_())